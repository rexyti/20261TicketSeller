...
class ZonaRepositoryAdapterTest {
        ...
        void setup() throws Exception {
                ...
                databaseClient.sql(
                                "INSERT INTO recintos (id,nombre,ciudad,direccion,capacidad_maxima,telefono,fecha_creacion,compuertas_ingreso,activo,categoria) VALUES (:id,:n,:c,:d,:cap,:t,now(),:comp,true,null)")
                                .bind("id", recintoId)
                                .bind("n", "Arena")
                                .bind("c", "Bogota")
                                .bind("d", "Calle 1")
        ...
        void deberiaPersistirZona() {
                Zona zona = Zona.builder().id(UUID.randomUUID()).recintoId(recintoId).nombre("VIP").capacidad(200)
                                .build();
