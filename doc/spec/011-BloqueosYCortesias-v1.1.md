# Feature Specification: Bloqueos y Cortesías

**Created**: 23/02/2026

## User Scenarios & Testing *(mandatory)*

---

### User Story 1 - Bloquear asientos para patrocinadores (Priority: P1)

Como **Coordinador de Patrocinios**, quiero poder bloquear asientos específicos para patrocinadores antes de que salgan
a la venta general, para cumplir con los acuerdos comerciales.

**Why this priority**: Es la función principal. Gracias a esto, se pueden reservar asientos para patrocinadores y se
cumplen los contratos.

**Independent Test**: El coordinador selecciona 20 asientos en la sección VIP, los marca como "Bloqueados para
patrocinador" y verifica que ya no aparecen disponibles en el mapa público.

**Acceptance Scenarios**:

1. **Scenario: Bloqueo exitoso de asientos**
    - **Given** que hay asientos disponibles en el mapa del evento
    - **When** el coordinador selecciona varios asientos y los bloquea para un patrocinador específico
    - **Then** los asientos aparecen como **BLOQUEADOS - PATROCINADOR** en el sistema interno y no son visibles ni
      seleccionables para el público general

---

### User Story 2 - Crear cortesías para invitados (Priority: P1)

Como **Coordinador de patrocinios**, quiero poder generar tickets de cortesía para invitados especiales, sin que pasen
por el proceso de pago, para facilitar la entrada de patrocinadores, prensa o artistas.

**Why this priority**: Es una funcionalidad crítica para relaciones públicas y operación del evento.

**Independent Test**: El coordinador crea 5 cortesías para "Prensa", asigna asientos específicos y verifica que aparecen
como "Cortesía" en el sistema y que pueden ser validadas en puerta.

**Acceptance Scenarios**:

1. **Scenario: Creación de cortesía con asiento asignado**
    - **Given** que hay asientos disponibles
    - **When** el coordinador crea una cortesía y le asigna un asiento específico
    - **Then** el asiento queda marcado como **Bloqueado** en el inventario y se genera el ticket para el invitado

2. **Scenario: Creación de cortesía sin asiento asignado**
    - **Given** que un recinto tiene área de sin asientos
    - **When** el coordinador crea un ticket de cortesía sin asiento fijo
    - **Then** se genera el ticket para el invitado y cuenta para el aforo del evento.

---

### User Story 3 - Gestionar bloqueos y cortesías desde panel (Priority: P2)

Como **Coordinador de Patrocinios**, quiero tener un panel donde pueda ver todos los asientos bloqueados y cortesías
creadas, para tener control total y evitar duplicaciones.

**Why this priority**: La visibilidad es crucial para evitar bloquear dos veces el mismo asiento o perder el control
del inventario.

**Independent Test**: El coordinador accede al panel, filtra por ***Bloqueados*** y ve la lista completa con
patrocinadores, fechas y estados.

**Acceptance Scenarios**:

1. **Scenario: Visualización de bloqueos activos**
    - **Given** que hay múltiples bloqueos y cortesías creadas
    - **When** el coordinador abre el panel de administración
    - **Then** ve una lista clara con asiento, tipo (bloqueo/cortesía), destinatario y fecha de expiración

2. **Scenario: Edición de bloqueo existente**
    - **Given** un bloqueo creado para un patrocinador
    - **When** el coordinador necesita cambiarlo a otro patrocinador
    - **Then** la etiqueta del bloqueo cambia sin cambiar el estado del asiento.

3. **Scenario: Liberación manual anticipada**
    - **Given** un bloqueo activo
    - **When** el patrocinador confirma que no usará los asientos
    - **Then** el coordinador puede liberarlos manualmente y pasan a disponible inmediatamente

---

### Edge Cases

- **¿Qué pasa si se bloquea un asiento que ya estaba bloqueado?**  
  El sistema debe alertar que el asiento ya tiene un bloqueo activo y no permitir duplicados.
- **¿Qué pasa si se crea una cortesía para un asiento ya ocupado?**  
  El sistema debe rechazar la operación y mostrar error.
- **¿Qué pasa si el patrocinador necesita más asientos de los disponibles en una sección?**  
  El coordinador debe poder ver disponibilidad en tiempo real y ajustar.
- **¿Qué pasa con bloqueos para eventos recurrentes?**  
  El sistema debe permitir copiar configuración de bloqueos de un evento a otro.
- **¿Qué pasa si un invitado con cortesía no llega?**  
  El asiento queda marcado como "Cortesía no usada" para reportes.
- **¿Qué pasa si se necesita transferir una cortesía a otra persona?**  
  El coordinador debe poder reasignar el ticket a otro nombre sin cambiar el asiento.
- **¿Qué pasa si un bloqueo no tiene fecha de liberación y el evento se acerca?**  
  El coordinador debe poder ver esos casos en el panel y decidir si liberar manualmente.

---

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: El sistema **DEBE** permitir bloquear asientos individuales o en grupo para patrocinadores específicos.
- **FR-002**: El sistema **DEBE** etiquetar cada bloqueo con el nombre del patrocinador o destinatario.
- **FR-003**: El sistema **DEBE** impedir que los asientos bloqueados sean visibles o comprables por el público general.
- **FR-004**: El sistema **DEBE** permitir crear tickets de cortesía con o sin asiento asignado.
- **FR-005**: El sistema **DEBE** generar un código único para cada cortesía creada.
- **FR-006**: El sistema **DEBE** tener un panel de administración que muestre todos los bloqueos y cortesías activos.
- **FR-007**: El sistema **DEBE** permitir editar la información de un bloqueo (destinatario) sin liberar el asiento.
- **FR-008**: El sistema **DEBE** permitir liberar manualmente bloqueos en cualquier momento.
- **FR-009**: El sistema **DEBE** validar que un asiento no esté ya bloqueado u ocupado antes de crear un nuevo bloqueo.
- **FR-010**: El sistema **DEBE** [NEEDS CLARIFICATION: definir límite máximo de bloqueos por evento? Por patrocinador?]

---

### Key Entities

- **Ticket**: Representa el comprobante digital de entrada para un asiento específico en un evento. Es el "producto"
  final que se compra. Se relaciona con Venta y contiene atributos como: Código QR único, Estado (Vendido, Reembolsado).
- **Evento**: Instancia del espectáculo. Atributos: Nombre, Fecha, Lugar.
- **Asiento**: Ubicación física. Atributos: ID, Ubicación, Estado (Disponible, Bloqueado, Ocupado, Cortesía).

---

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Bloquear 50 asientos toma menos de 2 minutos.
- **SC-002**: Cero casos donde un asiento bloqueado se venda al público.
- **SC-003**: El panel de administración carga la lista de bloqueos en menos de 3 segundos.
- **SC-004**: 95% de las cortesías tienen información completa de destinatario y categoría.
- **SC-005**: Cero reclamos de patrocinadores por asientos bloqueados incorrectamente.
